...
Tässä luvussa esitellyt Pythonin tietorakenteet käyttävät hajautusta, jossa on taustalla tietorakenne [hajautustaulu](../hajautustaulu). Pythonissa hajautustaulun toteutus perustuu avoimeen hajautukseen.

Pythonissa on sisäänrakennettu funktio `hash`, jolla voidaan laskea alkion hajautusarvo. Python kutsuu tätä funktiota, kun se määrittää alkion sijainnin tietorakenteissa `set` ja `dict`. Funktion toimintaa voi testata näin:
