...
Tässä `value` on solmuun liittyvä arvo ja lista `children` sisältää viittaukset solmun lapsiin. Oletuksena `children` on tyhjä lista eli solmulla ei ole lapsia.
...
Olemme määritelleet luokan `Node` niin, että alustusmetodissa parametrilla `children` on oletusarvona tyhjä lista:
...
Tämä toimii yleensä hyvin, mutta Pythonissa on yllättävä piirre, joka voi aiheuttaa ongelmia joissakin tilanteissa. Seuraava koodi esittelee asiaa:
...
Tämä kannattaa pitää mielessä Pythonissa aina, kun parametrin oletusarvona on olio, jonka sisältö voi muuttua. Näin on myös seuraavassa koodissa:
